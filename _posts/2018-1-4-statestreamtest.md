...
![]({{ site.url }}assets/imgs/sparktest_infiniteset.png?raw=true) 
...
![]({{ site.url }}assets/imgs/sparktest_infinitesetstate.png?raw=true)
...
<img src="{{ site.url }}/assets/imgs/sparktest_szero.png?raw=true" width="20" height="20" />
...
But what if we needed some more state to be built up, before we can be able to produce the current tick? Taking a step back, in this simple writing if you unfold the recurrence the state at time ![]({{ site.url }}assets/imgs/sparktest_t.png?raw=true), so ![]({{ site.url }}assets/imgs/sparktest_st.png?raw=true), is built as a function of the initial state and the events so far:

![]({{ site.url }}assets/imgs/sparktest_sigma.png?raw=true)

and what if this ![]({{ site.url }}assets/imgs/sparktest_sigma.png?raw=true) is not defined? Coming back to planet earth: have we seen enough events in the stream for the state to be defined? Say in the application we are asked: for a new car, determine the the driver profile (occasional, commuter, taxi driver etc) after no less than eight days of usage. Before eight days we could not really dare to say anything, the state is undefined!

Now in our testing scenario we are given some stream ![]({{ site.url }}assets/imgs/sparktest_xtest.png?raw=true) and the sequence of state sets ![]({{ site.url }}assets/imgs/sparktest_stest.png?raw=true) our application is expected to emit seeing that as an input. These two sequences are extracts of a run of the original application: think that the old application is running and at some point the input messages, and the state produced for those, are recorded.

Again, assume this eight-day horizon for the application. The very first eight shots of our stateful application, so the set ![]({{ site.url }}assets/imgs/sparktest_sacht.png?raw=true) will be undefined. To check whether our new implementation is correct or not, we have to evaluate the equality:

![]({{ site.url }}assets/imgs/sparktest_sold.png?raw=true)

for every time ![]({{ site.url }}assets/imgs/sparktest_t.png?raw=true). Until ![]({{ site.url }}assets/imgs/sparktest_totto.png?raw=true) this equality is not well defined: while the state recorded on the old app is legit, what we are producing on the side of the new app is not defined, because this is a cold start and we are missing whatever the old app had before. The eight time ticks is actually analogous to what in other areas is referred to as *burn-in period*.
...
The class `DriverState` represents the current state for a specific car / driver (let's use these terms interchangeably); all together, they make up the big state ![]({{ site.url }}assets/imgs/sparktest_st.png?raw=true) at the current time. Every time we see a new car beacon come in, there are two cases:
...
```
The `createStream` method is our entry point: it accepts the incoming stream of messages ![]({{ site.url }}assets/imgs/sparktest_bigx.png?raw=true) and creates a global state ![]({{ site.url }}assets/imgs/sparktest_bigS.png?raw=true). There are three operations:

1. define a method to update the state; this is our ![]({{ site.url }}assets/imgs/sparktest_sigmello.png?raw=true) function
...
There are many other functionalities Spark supports here, for example setting a timeout after which long-not-seen entries are discarded, choosing an initial state ![]({{ site.url }}assets/imgs/sparktest_szero.png?raw=true). Spark requires the input stream to be key-paired in order to let us define a state, a state instance is always linked to a key entry.
...
1. generates a number of random records for the ![]({{ site.url }}assets/imgs/sparktest_xtest.png?raw=true) stream
2. computes a full test state ![]({{ site.url }}assets/imgs/sparktest_soldsing.png?raw=true) and serializes it
3. defines a function ![]({{ site.url }}assets/imgs/sparktest_sold.png?raw=true) to test the performance of the "new" streamer against the data of the full test state, ideally produced by some "old" implementation
4. properly configures the testing framework
5. executes the run with a specified burn-in, which is creating a ![]({{ site.url }}assets/imgs/sparktest_sigma.png?raw=true and testing it with the equality

If we ask our new stream processor to calculate a state from the input file, the result would be completely different from the test state, until some time ![]({{ site.url }}assets/imgs/sparktest_t.png?raw=true) is hit. Before this point, an equality function ![]({{ site.url }}assets/imgs/sparktest_sold.png?raw=true) is not defined - this is until we stay in the famous initial sequence - we have to discard this.
